...
def render() -> None:
    ...
    with st.expander("➕ Cadastrar Novo Sócio"):
        ...
        with n3:
            ...
            if st.button("Adicionar ✚", use_container_width=True):
                if not _is_admin():
                    st.error("🔒 Operação não permitida.")
    ...
    tab_todos, tab_inad, tab_cobrancas = st.tabs([
        "📋 Todos os Sócios",
...
def _render_tabela_editavel(df: pd.DataFrame) -> None:
    ...
    # height fixo = sem rolagem da página inteira
    edited = st.data_editor(
        ...
        height=min(400, 56 + len(df) * 35),   # cresce com nº de sócios, teto 400px
    )
    ...
    if _is_admin():
        ...
        st.caption("🔒 Edição desativada — acesso somente leitura.")


def _render_inadimplentes(df: pd.DataFrame, mes_atual: str) -> None:
    """
    Substituímos o loop de colunas+dividers por uma tabela HTML compacta.
    Isso elimina o scroll longo quando há muitos inadimplentes.
    """
    ...
    st.warning(f"**{len(df_inad)}** sócio(s) com mensalidade pendente em **{mes_atual}**.")
    ...
    for _, row in df_inad.iterrows():
        ...
        link = _gerar_link_wpp(tel, nome, meses_p)
        btn  = (
            f'<a href="{link}" target="_blank" class="wpp-btn">📱 WhatsApp</a>'
            if link
            else '<span style="color:#8899AA;font-size:0.78rem">Sem telefone</span>'
        )
        meses_label = ", ".join(meses_p) if meses_p else "—"
        rows_html += (
            f"<tr>"
            f"<td style='padding:7px 10px;font-weight:600'>{nome}</td>"
            f"<td style='padding:7px 10px;color:#E8C878;font-size:0.82rem'>{meses_label}</td>"
            f"<td style='padding:7px 10px'>{btn}</td>"
            f"</tr>"
        )

    st.markdown(
        f"""
        <div style="overflow-y:auto;max-height:380px;border:1px solid rgba(201,168,76,0.18);
                    border-radius:10px;">
        <table style="width:100%;border-collapse:collapse;font-size:0.88rem;">
          <thead style="position:sticky;top:0;background:#0C1A31;z-index:1;">
            <tr style="color:#C9A84C;text-align:left;border-bottom:1px solid rgba(201,168,76,0.3);">
              <th style="padding:8px 10px">Nome</th>
              <th style="padding:8px 10px">Meses pendentes</th>
...
def _render_central_cobrancas(df: pd.DataFrame) -> None:
    ...
    mes_sel = st.selectbox(
        "Mês de referência", options=gs.MESES,
    ...
    for _, row in df_inad.iterrows():
        ...
        btn  = (
            ...
            if link
            else '<span style="color:#8899AA;font-size:0.8rem">Sem tel.</span>'
        )
        rows_html += (
            f"<tr>"
            f"<td style='padding:7px 10px'>{nome}</td>"
            f"<td style='padding:7px 10px'>{len(meses_p)} meses</td>"
            f"<td style='padding:7px 10px'>{btn}</td>"
            f"</tr>"
        )
    ...
    st.markdown(
        ...
        <table style="width:100%;border-collapse:collapse;font-size:0.9rem;">
          <thead style="position:sticky;top:0;background:#0C1A31;z-index:1;">
            <tr style="color:#C9A84C;text-align:left;border-bottom:1px solid rgba(201,168,76,0.3);">
              <th style="padding:8px 10px">Nome</th>
              <th style="padding:8px 10px">Meses Devendo</th>
              <th style="padding:8px 10px">Ação</th>
            </tr>
          </thead>
          <tbody>{rows_html}</tbody>
